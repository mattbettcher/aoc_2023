    ...
    pub fn compute_with_joker(&mut self) {
        ...
        if map.contains_key(&Card::Joker) {
            ...
            match (jokers, map.len()) {
                ...
                // only one other card and 4 jokers
                (_, 1) => {
                    let c = self.cards[0];
        ...
        match map.len() {
            ...
            // either 4 + 1 or 3 + 2
            2 => match &card_counts[..] {
                [(_, 1), (_, 4)] => self.kind = HandType::FourOfAKind,
                [(_, 2), (_, 3)] => self.kind = HandType::FullHouse,
                _ => unreachable!(),
            },
            // found 3 types of cards in the hand
            // either 2 + 2 + 1 or 3 + 1 + 1
            3 => match &card_counts[..] {
                [(_, 1), (_, 2), (_, 2)] => self.kind = HandType::TwoPair,
                [(_, 1), (_, 1), (_, 3)] => self.kind = HandType::ThreeOfAKind,
                _ => unreachable!(),
            },
            // found 4 types of cards in the hand
            // 2 + 1 + 1 + 1
